...
function MainSettings(p) {
  ...
  const settings= {
    "Currency": ["United States (USD)","Indonesia (IDR)","Japan (JPY)","Russia (RUB)","Germany (EUR)","Korea (WON)","Egyptian (LE)"],
    "Language": ["English (EN)","Indonesian (ID)","Arabic (AR)","Chinese (ZH)","Chinese (ZH)","French (FR)","German (DE)","Italian (IT)","Korean (KO)","Portuguese (PT)","Russian (RU)","Spanish (ES)"],
    "Theme": ["Dark","Light"],
    "Security": ["PIN","Fingerprints","Face ID"],
    "Notification":[["Expense Alert","Get notification about you’re expense"],
      ...
      <Stack anim={"stack-right"} on={p.setting2} set={p.setting2_set}>
        <Page pad={"pt-16"} bar={
          <Topbar cls="flex-row h-16 m-auto">
            <img class="z-10" src="/arrow-left.svg" onClick={()=> p.setting2_set(false)}></img>
          ...
          <For each={Object.keys(settings)} fallback={<div>Loading...</div>}>
            {(a) => <Item2names text={a} act={()=> settingsfn[a](true)}  text2={a != "Notification" ? settings[a][sad()[a]] : ""}></Item2names>}
          </For>
        </Page>
      </Stack>
      <For each={Object.keys(settings)} fallback={<div>Loading...</div>}>
        {(a,i) => 
          <Stack anim={"stack-right"} on={getfn[a]} set={settingsfn[a]}>
            <Page pad={"pt-16"} bar={
              <Topbar cls="flex-row h-16 m-auto">
                <img class="z-10" src="/arrow-left.svg" onClick={()=> settingsfn[a](false)}></img>
                <h1 className="fixed w-full text-center m-auto SfProBold dark:text-white text-2xl">{a}</h1>
              </Topbar>
            }>
