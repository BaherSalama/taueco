function add_code(e) {
	...
	this.style.setProperty(
		"--underline-offset-x",
		`${encodeURIComponent.target.offsetLeft}px`,
	);
}
...
function Ele(props) {
	return (
		<li
			...
			classList={{ pageon: props.tab() === props.to }}
			onClick={props.a.updateTab(props.to)}
		>
			<a class="flex flex-col">
				...
				<p class="SfProMeduim text-gray-500 text-center text-xs">
					{props.name}
				</p>
			</a>
		</li>
		...
		<div class="left-1/2 fixed bottom-6 h-20 w-full bg-white BarShadow rounded-full -translate-x-1/2 px-5">
			<ul class="size-full flex row menu__list mx">
				...
				<li class="size-5/6 m-auto" onClick={() => a.seton(true)}>
					<img src="add-circle.svg" class="mx-auto h-full ">
						{" "}
					</img>
				</li>
